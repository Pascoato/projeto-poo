...
public class BankingService {
    ...
    public BankingService(IUserInterFace iUserInterFace, IUserRepository userRepository, IBassRepository iBassRepository) {
        this.iUserInterFace = iUserInterFace;
        this.userDB = userRepository;
        this.iBassRepository = iBassRepository;
    }

    public Integer apresentarMeuInicial() {
    ...
    public UserAccountDTO findUser(LoginDTO loginDTO) {
        ...
        if (!user.getPassword().equals(loginDTO.getPassaword())) {
            iUserInterFace.showErrorMenssagem("Credencial Inválida!");
            return null;
        }
        return user;
    }
